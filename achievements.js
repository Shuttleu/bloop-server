...
async function digitSum(user){
    ...
    if (lastBloop[0].UserId == previousBloop[0].UserId){
        return false;
    }

    let lastId = lastBloop[0].UserId;
    let lastSum = 0;
    let previousId = previousBloop[0].UserId;
    let previousSum = 0;
...
async function firstBork(user){
    return await user.countBloops() == 1;
}
...
async function nightOwl(user){
    ...
    return lastBloop[0].createdAt.getHours() > 11 || lastBloop[0].createdAt.getHours() < 3;
}
