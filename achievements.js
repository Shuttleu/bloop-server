...
async function alphabetically(user){
  ...
  const bloops = await user.getBloops({order: [['createdAt', 'DESC']], limit: count});
  let alphabetical = true;
  bloops.forEach((bloop, index) => {
    if (bloop.BoxId != boxes[index].id)
      alphabetical = false;
  });
  return alphabetical;
}
...
async function digitSum(user){
  const lastBloop = await user.getBloops({order: [['createdAt', 'DESC']], limit: 1, include: ["User"]})
  console.log("User");
  console.log(lastBloop[0].User);
  const previousBloop = await db.Bloop.findAll({where: {id: {[Op.lte]: lastBloop[0].id}, BoxId: lastBloop[0].BoxId}, order: [['createdAt', 'DESC']], limit: 1, offset: 1, include: ["User"]})
...
async function earlyBird(user){
  const lastBloop = await user.getBloops({order: [['createdAt', 'DESC']], limit: 1})

  return lastBloop[0].createdAt.getHours() > 4 && lastBloop[0].createdAt.getHours() < 7;
}
async function firstBloopOfTheHour(user){
  const lastBloop = await user.getBloops({order: [['createdAt', 'DESC']], limit: 1})
  const previousBloop = await db.Bloop.findAll({where: {id: {[Op.lte]: lastBloop[0].id}, BoxId: lastBloop[0].BoxId}, order: [['createdAt', 'DESC']], limit: 1, offset: 1})

  return lastBloop[0].createdAt.getHours() != previousBloop[0].createdAt.getHours();
}
async function highNoon(user){
  const lastBloop = await user.getBloops({order: [['createdAt', 'DESC']], limit: 1})

  return lastBloop[0].createdAt.getHours() == 12 && lastBloop[0].createdAt.getMinutes() == 0;
}
async function leet(user){
  const lastBloop = await user.getBloops({order: [['createdAt', 'DESC']], limit: 1})

  return lastBloop[0].createdAt.getHours() == 13 && lastBloop[0].createdAt.getMinutes() == 37;
}
async function nightOwl(user){
  const lastBloop = await user.getBloops({order: [['createdAt', 'DESC']], limit: 1})

  return lastBloop[0].createdAt.getHours() > 23 || lastBloop[0].createdAt.getHours() < 3;
}
async function e621(user){
  const lastBloop = await user.getBloops({order: [['createdAt', 'DESC']], limit: 1})

  return (lastBloop[0].createdAt.getHours() == 6 || lastBloop[0].createdAt.getHours() == 18) && lastBloop[0].createdAt.getMinutes() == 21;
}
